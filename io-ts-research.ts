 ...
 * @since 1.0.0
 */
export class RefinementType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
 ...
 * @since 1.8.1
 */
export const Int = brand(
  number,
  (n): n is Branded<number, IntBrand> => Number.isInteger(n),
  "Int"
);
 ...
 * @since 1.0.0
 */
export class KeyofType<D extends { [key: string]: unknown }>
 ...
 * @since 1.5.3
 */
export interface KeyofC<D extends { [key: string]: unknown }>
  extends KeyofType<D> {}
...
export const keyof = <D extends { [key: string]: unknown }>(
  ...
  const is = (u: unknown): u is keyof D =>
    string.is(u) && hasOwnProperty.call(keys, u);
 ...
 * @since 1.0.0
 */
export class RecursiveType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
 ...
 * @since 1.0.0
 */
export class ArrayType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
      ...
      chain(UnknownArray.validate(u, c), (us) => {
        ...
        for (let i = 0; i < len; i++) {
          ...
          const result = codec.validate(
            ui,
            appendContext(c, String(i), codec, ui)
          );
          if (isLeft(result)) {
 ...
 * @since 1.0.0
 */
export class InterfaceType<P, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
      ...
      chain(UnknownRecord.validate(u, c), (o) => {
        ...
        for (let i = 0; i < len; i++) {
          ...
          if (isLeft(result)) {
            ...
            if (
              vak !== ak || (vak === undefined && !hasOwnProperty.call(a, k))
            ) {
 ...
 * @since 1.0.0
 */
export class PartialType<P, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
 ...
 * @internal
 */
export function getDomainKeys<D extends Mixed>(domain: D): Record<
  string,
  unknown
> | undefined {
    ...
    (u, c) => {
      if (UnknownRecord.is(u)) {
        ...
        for (let i = 0; i < len; i++) {
          let k = keys[i];
          const ok = u[k];
          const domainResult = domain.validate(
            k,
 ...
 * @since 1.0.0
 */
export class UnionType<CS extends Array<Any>, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
...
const getUnionName = <CS extends [Mixed, Mixed, ...Array<Mixed>]>(
  codecs: CS
): string => {
    ...
    CS extends { length: 2 } ? TypeOf<CS[0]> & TypeOf<CS[1]>
      ...
      : CS extends { length: 5 }
        ? TypeOf<CS[0]> & TypeOf<CS[1]> & TypeOf<CS[2]> & TypeOf<CS[3]>
          & TypeOf<CS[4]>
      : unknown,
    CS extends { length: 2 } ? OutputOf<CS[0]> & OutputOf<CS[1]>
      : CS extends { length: 3 }
 ...
 * @since 1.0.0
 */
export class TupleType<CS extends Array<Any>, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
 ...
 * @since 1.5.3
 */
export interface TupleC<CS extends [Mixed, ...Array<Mixed>]> extends TupleType<
  CS,
  CS extends { length: 1 } ? [TypeOf<CS[0]>]
    ...
    : unknown,
  CS extends { length: 1 } ? [OutputOf<CS[0]>]
    : CS extends { length: 2 } ? [OutputOf<CS[0]>, OutputOf<CS[1]>]
    : CS extends { length: 3 }
      ? [OutputOf<CS[0]>, OutputOf<CS[1]>, OutputOf<CS[2]>]
    : CS extends { length: 4 }
...
export function tuple<A extends Mixed>(codecs: [A], name?: string): TupleC<
  [A]
>;
export function tuple<CS extends [Mixed, ...Array<Mixed>]>(
  ...
  return new TupleType(
    name,
    (u): u is any =>
      UnknownArray.is(u) && u.length === len && codecs.every((type, i) =>
        type.is(u[i])
      ),
    (u, c) =>
      chain(UnknownArray.validate(u, c), (us) => {
        ...
        for (let i = 0; i < len; i++) {
          ...
          const result = type.validate(
            a,
            appendContext(c, String(i), type, a)
          );
          if (isLeft(result)) {
            pushAll(errors, result.left);
          } else {
    ...
    useIdentity(codecs)
      ? identity
      : (a) => codecs.map((type, i) => type.encode(a[i])),
    codecs
  );
}
 ...
 * @since 1.0.0
 */
export class ReadonlyType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
 ...
 * @since 1.5.3
 */
export interface ReadonlyC<C extends Mixed> extends ReadonlyType<
  C,
 ...
 * @since 1.0.0
 */
export const readonly = <C extends Mixed>(
  codec: C,
  name: string = `Readonly<${codec.name}>`
): ReadonlyC<C> => {
 ...
 * @since 1.0.0
 */
export class ReadonlyArrayType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
 ...
 * @since 1.5.3
 */
export interface ReadonlyArrayC<C extends Mixed> extends ReadonlyArrayType<
  C,
 ...
 * @since 1.0.0
 */
export const strict = <P extends Props>(props: P, name?: string): ExactC<
  TypeC<P>
> => {
...
> extends UnionType<CS, A, O, I> {
  constructor(
    ...
    super(name, is, validate, encode, codecs); // <= workaround for https://github.com/Microsoft/TypeScript/issues/13455
    /* istanbul ignore next */
  }
}
...
> // tslint:disable-next-line: deprecation
  extends
    TaggedUnionType<Tag, CS, TypeOf<CS[number]>, OutputOf<CS[number]>, unknown>
{}
...
>(
  ...
  if (U instanceof TaggedUnionType) {
    ...
    console.warn(
      `[io-ts] Cannot build a tagged union for ${name}, returning a de-optimized union`
    );
    // tslint:disable-next-line: deprecation
 ...
 * @since 1.1.0
 */
export class ExactType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  /**
   * @since 1.0.0
   */
 ...
 * @since 1.1.0
 */
export type HasProps = HasPropsRefinement
  | HasPropsReadonly
  ...
  return new ExactType(
    ...
    (u, c) =>
      chain(
        ...
        () => map(codec.validate(u, c), (a) => stripKeys(a, props))
      ),
 ...
 * @deprecated
 */
export const getValidationError /* istanbul ignore next */ = (
  value: unknown,
  context: Context
): ValidationError => ({
  value,
 ...
 * @deprecated
 */
export const getDefaultContext /* istanbul ignore next */ = (
  decoder: Decoder<any, any>
): Context =>
  [
    { key: "", type: decoder }
...
RefinementC<C> {
  return new RefinementType(
    ...
    (i, c) =>
      chain(
        codec.validate(i, c),
        (a) => (predicate(a) ? success(a) : failure(a, c))
      ),
    codec.encode,
    codec,
    predicate
 ...
 * @deprecated
 */
export type TaggedIntersectionArgument<Tag extends string> = // tslint:disable-next-line: deprecation
[Tagged<Tag>]
  | // tslint:disable-next-line: deprecation
   [Tagged<Tag>, Mixed]
  | // tslint:disable-next-line: deprecation
 ...
 * @deprecated
 */
export type Tagged<Tag extends string, A = any, O = A> = // tslint:disable-next-line: deprecation
 ...
 * @deprecated
 */
export function clean<A, O = A, I = unknown>(codec: Type<A, O, I>): Type<
  A,
  O,
  I
> {
...
function isIntersectionC(codec: Any): codec is IntersectionC<
  [Mixed, Mixed, ...Array<Mixed>]
> {
  return (codec as any)._tag === "IntersectionType";
}

function isUnionC(codec: Any): codec is UnionC<
  [Mixed, Mixed, ...Array<Mixed>]
> {
...
export function getTags(codec: Any): Tags {
  ...
  if (isTypeC(codec) || isStrictC(codec)) {
    ...
    return codec.types.slice(1).reduce(
 ...
 * @internal
 */
export function getIndex(codecs: NonEmptyArray<Any>): [
  string,
  NonEmptyArray<NonEmptyArray<LiteralValue>>
] | undefined {
...
const decoder = union([literal("a"), literal(2)]);
type InferredAsf = TypeOf<typeof decoder>;
